...
The coordinate reference system (CRS) used for this dataset is: "`r st_crs(ca_subset)$input`"
...
new_spill_data <- st_transform(spill_data, 3857)

# Then check it: 
#new_spill_data %>% st_crs()
```

Note that this dataset uses a different coordinate reference system (CRS) ("`r st_crs(spill_data)$input`") from the state spatial data, so we need to update it so they match (both "`r st_crs(new_spill_data)$input`"). 
...
```{r spills by county, fig.cap="**Figure 2.** Chloropleth showing counties by spill frequency"}
